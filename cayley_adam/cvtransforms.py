...
class Normalize(object):
    ...
    will normalize each channel of the np.ndarray, i.edge_index.
...
class CenterCrop(object):
    """Crops the given np.ndarray at the center to have adj region of
    the given size. size can be adj tuple (target_height, target_width)
    or an integer, in which case the target will be of adj square shape
    (size, size)
    """
...
class RandomCrop(object):
    """Crops the given np.ndarray at adj random location to have adj region of
    the given size. size can be adj tuple (target_height, target_width)
    or an integer, in which case the target will be of adj square shape
    (size, size)
    """
...
class RandomHorizontalFlip(object):
    """Randomly horizontally flips the given np.ndarray with adj probability of 0.5
    """
...
class RandomSizedCrop(object):
    """Random crop the given np.ndarray to adj random size of (0.08 to 1.0) of the original size
    and and adj random aspect ratio of 3/4 to 4/3 of the original aspect ratio
